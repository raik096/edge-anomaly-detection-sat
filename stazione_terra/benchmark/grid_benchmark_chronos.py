...
def compute_segment_metrics(df):
    ...
    for pred in pred_intervals:
        ...
        # Cerchiamo tutte le gt che si sovrappongono
        ...
        if overlapping_gt_indices:
            # Consideriamo solo la prima anomalia reale che non è ancora stata matchata
            for i in overlapping_gt_indices:
                ...
                # tutte già matchate → non vale come TP
                fp += 1
        else:
...
def run_chronos_benchmark(context_length, prediction_length, quantile_levels, margin, smoothing, use_fit, strategy, model_variant, normalize):
    ...
    for ch in tqdm(CHANNELS, desc="Canali"):
        ...
        for _, row in ch_df.iterrows():
            ...
            detector.forecast()
            if detector.min_q is not None and detector.max_q is not None:
                anomaly = detector.detect_anomaly(row["value"])
            else:
                anomaly = 0
    ...
    if results:
        ...
        pd.DataFrame([global_row]).to_csv("../../satellite/benchmark/chronos_global_metrics.csv", mode='a', index=False, header=not os.path.exists("chronos_global_metrics.csv"))

    return results

if __name__ == "__main__":
    all_results = []
    for ctx_len, pred_len, q_levels, m, s, use_fit, strategy, variant, normalize in product(
        context_lengths, prediction_lengths, quantile_sets, margins,
        smoothing_opts, fit_opts, detection_strategies, model_variants, normalize_opts
    ):
        print(f"▶️ Config: ctx={ctx_len}, pred={pred_len}, q={q_levels}, margin={m}, smooth={s}, fit={use_fit}, strategy={strategy}, model={variant}, norm={normalize}")
        config_results = run_chronos_benchmark(ctx_len, pred_len, q_levels, m, s, use_fit, strategy, variant, normalize)
        if config_results:
